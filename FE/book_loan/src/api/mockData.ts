...
export async function mockAddBook(payload: BookPayload) {
  ...
  const newBook: RawBook = {
    ...
    genre: normalizePhysicalCategory(payload.category || payload.genre),
    published_year: payload.published_year,
    cover: payload.cover || DEFAULT_COVER,
    location: normalizePhysicalLocation(payload.category || payload.genre, payload.location),
    total_quantity: qty,
...
export async function mockUpdateBook(bookId: number, payload: BookPayload) {
  ...
  const updated: RawBook = {
    ...
    genre: normalizePhysicalCategory(payload.category || payload.genre || current.genre),
    published_year: payload.published_year ?? current.published_year,
    cover: payload.cover || current.cover,
    location: normalizePhysicalLocation(
      payload.category || payload.genre || current.genre,
